...
def extract_video_frames (video_path, frame_path):
    ...
    print(video_path)
    print(frame_path)
    os.system('ffmpeg -i ' + video_path + ' -q:v 2 -f image2 ' + os.path.abspath(os.path.join(os.path.join(BASE_DIR, frame_path), '/image_%5d.jpg')))
    return
...
def make_frame_dir (frame_dir):
    frame_dir = os.path.join(BASE_DIR, frame_dir)
